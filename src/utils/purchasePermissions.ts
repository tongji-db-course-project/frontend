...
export const getCurrentRoleName = () => {
  ...
  const roleName = normalizeRoleName(authStore.userInfo?.roleName) || normalizeRoleName(localStorage.getItem('currentRoleName'))
  return roleName
}
...
// 三账号职责：采购员制单及执行入库，管理员独立审批。
export const canCreatePurchase = () => hasAnyRole('采购员', '管理员')
export const canApprovePurchase = () => hasAnyRole('管理员')
export const canStockInPurchase = () => hasAnyRole('采购员', '管理员')

// 可编辑状态：待审批（提交前）、已驳回（被打回修改）
export const canEditPurchaseBeforeApproval = (status?: string) =>
  (status === '待审批' || status === '已驳回') && hasAnyRole('采购员', '管理员')

// 可作废状态：待审批/已驳回/已审批；已入库、已作废不可作废（后端一致）
export const canCancelPurchaseBeforeApproval = (status?: string) =>
  (status === '待审批' || status === '已驳回' || status === '已审批') && hasAnyRole('采购员', '管理员')

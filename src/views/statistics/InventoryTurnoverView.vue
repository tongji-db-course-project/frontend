  ...
  <div class="biz-page">
    ...
    <section class="biz-card filter-card">
      <div class="range-row">
        ...
        <template v-if="query.periodType === 'month' || query.periodType === 'quarter' || query.periodType === 'halfYear' || query.periodType === 'year'">
          <el-select v-model="periodYear" class="period-year" placeholder="年份">
            <el-option v-for="year in yearOptions" :key="year" :label="`${year}年`" :value="year" />
          </el-select>
        </template>
        <el-select v-if="query.periodType === 'month'" v-model="periodMonth" class="period-unit" placeholder="月份">
          <el-option v-for="month in 12" :key="month" :label="`${month}月`" :value="month" />
        </el-select>
        <el-select v-if="query.periodType === 'quarter'" v-model="periodQuarter" class="period-unit" placeholder="季度">
      ...
      <div class="threshold-row">
        ...
        <el-switch v-model="stagnantOnly" active-text="只看呆滞商品" />
      </div>
    </section>
    ...
    <section class="biz-card table-card">
      <el-table v-loading="loading" :data="displayItems" stripe border class="biz-table" empty-text="暂无符合条件的数据">
...
const displayItems = computed(() => stagnantOnly.value ? items.value.filter(item => item.stagnant) : items.value)
const slowCount = computed(() => displayItems.value.filter(item => item.status === 'slow').length)
const agedCount = computed(() => displayItems.value.filter(item => item.status === 'aged').length)
const stagnantCount = computed(() => displayItems.value.filter(item => item.stagnant).length)
...
const statusText = (status: string) => ({ normal: '正常', slow: '慢周转', aged: '呆滞' }[status] ?? '-')
const statusType = (status: string) => ({ normal: 'success', slow: 'warning', aged: 'danger' }[status] ?? 'info')
...
async function load() {
  ...
  loading.value = true
  try {
    const result = await statisticsApi.getInventoryTurnover({ ...query })
...
.period-type { width: 100px; }
.period-year { width: 110px; }
.period-unit { width: 120px; }
...
.threshold-item :deep(.el-input-number) { width: 120px; }

<template>
  <div class="biz-page">
    <PageHeader eyebrow="数据中心 · 库存效率" title="库存周转率" description="识别周转缓慢和呆滞积压商品" />
    <section class="biz-card toolbar">
      <el-date-picker v-model="range" type="daterange" value-format="YYYY-MM-DD" /><el-switch
        v-model="stagnantOnly"
        active-text="只看呆滞商品"
    ...
    <section class="biz-card">
      <el-table v-loading="loading" :data="displayRows" border
        ...
        /><el-table-column prop="beginningStock" label="期初库存" width="110" align="right" /><el-table-column
          ...
          width="110"
          align="right"
        /><el-table-column prop="averageStock" label="平均库存" width="110" align="right" /><el-table-column
          label="周转次数"
          width="115"
          align="right"
          ><template #default="{ row }"
            ><b>{{ Number(row.turnoverTimes).toFixed(2) }}</b></template
          ></el-table-column
        ><el-table-column label="识别结果" width="110" align="center"
          ><template #default="{ row }"
            ><el-tag :type="row.stagnant ? 'danger' : 'success'">{{ row.stagnant ? '呆滞' : '正常' }}</el-tag></template
          ></el-table-column
        ></el-table
      >
    </section>

  ...
  <div class="biz-page">
    ...
    <el-dialog v-model="createVisible" title="新建退货申请" width="680px">
      ...
      <el-form label-position="top" style="margin-top:14px">
        <div class="biz-form-grid"><el-form-item label="原销售单 ID" required><el-input-number v-model="createForm.saleId" :min="1" controls-position="right" /></el-form-item><el-form-item label="退货原因"><el-input v-model="createForm.remark" maxlength="200" /></el-form-item></div>
      </el-form>
      <div class="return-lines"><header><b>退货商品</b><el-button link type="primary" :icon="Plus" @click="addLine">添加一行</el-button></header><el-table :data="createForm.items" border size="small"><el-table-column label="商品 ID"><template #default="{ row }"><el-input-number v-model="row.productId" :min="1" controls-position="right" /></template></el-table-column><el-table-column label="退货数量"><template #default="{ row }"><el-input-number v-model="row.quantity" :min="1" controls-position="right" /></template></el-table-column><el-table-column width="70"><template #default="{ $index }"><el-button link type="danger" @click="createForm.items.splice($index,1)">删除</el-button></template></el-table-column></el-table></div>
...
const createForm = reactive<CreateReturnPayload>({ saleId: Number(route.query.saleId) || 0, remark: '', items: [{ productId: 0, quantity: 1 }] })
...
function openCreate() { if (!createForm.items.length) createForm.items.push({ productId: 0, quantity: 1 }); createVisible.value = true }
function addLine() { createForm.items.push({ productId: 0, quantity: 1 }) }
async function submitReturn() { const items = createForm.items.filter(item => item.productId > 0 && item.quantity > 0); if (createForm.saleId <= 0 || !items.length) { ElMessage.warning('请填写原销售单 ID 和至少一项退货商品'); return } submitting.value = true; try { await returnApi.create({ saleId: createForm.saleId, remark: createForm.remark || undefined, items }); ElMessage.success('退货申请已提交'); createVisible.value = false; Object.assign(createForm, { saleId: 0, remark: '', items: [{ productId: 0, quantity: 1 }] }); await load() } finally { submitting.value = false } }
onMounted(() => { load(); if (route.query.saleId) openCreate() })
</script>

<style scoped>.return-lines{margin-top:4px}.return-lines header{padding:8px 0;display:flex;align-items:center;justify-content:space-between}.return-lines :deep(.el-input-number){width:100%}</style>

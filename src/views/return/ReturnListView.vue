<template>
  <div class="biz-page">
    <PageHeader eyebrow="销售管理 · 售后业务" title="退货管理" description="收银员发起 → 系统管理员审核 → 采购员验收入库并完成退款与积分冲销">
      <el-button v-if="canCreate" type="primary" :icon="Plus" @click="openCreate">新建退货</el-button>
    </PageHeader>
    ...
    <section class="biz-card">
      <div class="biz-toolbar">
        ...
        <el-select v-model="query.status" placeholder="全部状态" clearable><el-option v-for="item in ['待处理','已审核','已完成','已拒绝']" :key="item" :label="item" :value="item" /></el-select>
      ...
      <el-table v-loading="loading" :data="orders" row-key="returnId" stripe border class="biz-table">
        ...
        <el-table-column label="操作" width="200" fixed="right" align="center"><template #default="{ row }">
          ...
          <template v-if="row.status === '待处理' && canReview">
            <el-popconfirm title="确认批准该退货申请？批准后将等待仓管办理入库。" confirm-button-text="批准" cancel-button-text="取消" width="260" @confirm="approve(row)">
          ...
          <el-popconfirm v-if="row.status === '已审核' && canStockIn" title="确认商品已验收并办理退货入库？" confirm-button-text="入库" cancel-button-text="取消" width="260" @confirm="complete(row)">
            <template #reference><el-button link type="success">确认入库</el-button></template>
          </el-popconfirm>
    ...
    <el-dialog v-model="createVisible" title="新建退货申请" width="680px">
      ...
      <el-form label-position="top" style="margin-top:14px">
        <div class="biz-form-grid"><el-form-item label="原销售单号或 ID" required><div class="sale-id-field"><el-input v-model.trim="createForm.saleIdentifier" placeholder="例如 SO2026... 或 12" @keyup.enter="loadSourceSale" /><el-button :loading="saleLoading" @click="loadSourceSale">读取销售单</el-button></div><small class="biz-muted">可直接粘贴会员消费轨迹中显示的销售单号</small></el-form-item><el-form-item label="退货原因"><el-input v-model="createForm.remark" maxlength="200" /></el-form-item></div>
      </el-form>
...
import { canCreateSalesReturn, canReviewSalesReturn, canStockInSalesReturn } from '../../utils/returnPermissions'

interface ReturnDraftItem { productId: number; quantity: number; refundPrice: number; maxReturnable: number }
interface ReturnDraft { saleIdentifier: string; saleId: number; remark: string; items: ReturnDraftItem[] }

const route = useRoute(), router = useRouter(), orders = ref<ReturnOrder[]>([]), loading = ref(false), total = ref(0)
...
const initialSaleIdentifier = String(route.query.saleNo || route.query.saleId || '')
const createForm = reactive<ReturnDraft>({ saleIdentifier: initialSaleIdentifier, saleId: 0, remark: '', items: [] })
const sourceSale = ref<SaleOrder | null>(null), saleLoading = ref(false)
const canCreate = computed(() => canCreateSalesReturn())
...
const statusTone = (status: ReturnStatus) => status === '已完成' ? 'green' : status === '待处理' ? 'orange' : status === '已审核' ? 'purple' : 'red'
...
async function approve(row: ReturnOrder) {
  try { await returnApi.approve(row.returnId); ElMessage.success(`退货单 ${row.returnNo} 已审核，等待入库`); await load() }
  catch (e: any) { ElMessage.error(e?.response?.data?.message || '批准失败，请稍后重试') }
}
async function complete(row: ReturnOrder) {
  try { await returnApi.complete(row.returnId); ElMessage.success(`退货单 ${row.returnNo} 已入库并完成退款`); await load() }
...
async function submitReject() {
  ...
  try { await returnApi.reject(rejectTarget.value.returnId, rejectRemark.value || null); ElMessage.success('已拒绝该退货申请'); rejectVisible.value = false; rejectTarget.value = null; await load() }
  catch (e: any) { ElMessage.error(e?.response?.data?.message || '拒绝失败，请稍后重试') }
  finally { rejectLoading.value = false }
}
function openCreate() {
  if (!canCreate.value) { ElMessage.warning('只有收银员可以发起销售退货'); return }
  createVisible.value = true
  if (createForm.saleIdentifier) void loadSourceSale()
}
const sourceItem = (productId: number) => sourceSale.value?.items?.find(item => item.productId === productId)
async function loadSourceSale() {
  const identifier = createForm.saleIdentifier.trim()
  if (!identifier) { ElMessage.warning('请先填写销售单号或 ID'); return }
  saleLoading.value = true; sourceSale.value = null; createForm.items.splice(0)
  try {
    let sale: SaleOrder
    if (/^\d+$/.test(identifier)) {
      sale = await saleApi.getDetail(Number(identifier))
    } else {
      const result = await saleApi.getList({ page: 1, size: 20, keyword: identifier })
      const matched = (result?.list ?? []).find(item => item.saleNo?.toLowerCase() === identifier.toLowerCase())
      if (!matched) { ElMessage.warning('未找到完全匹配的销售单号'); return }
      sale = await saleApi.getDetail(matched.saleId)
    }
...
async function submitReturn() {
  ...
  try {
    await returnApi.create({ saleId: createForm.saleId, memberId: sourceSale.value.memberId ?? null, remark: createForm.remark || null, details })
    ElMessage.success('退货申请已提交')
    createVisible.value = false; Object.assign(createForm, { saleIdentifier: '', saleId: 0, remark: '', items: [] })
    sourceSale.value = null; await load()
  } finally { submitting.value = false }
}
onMounted(() => { load(); if (initialSaleIdentifier && canCreate.value) openCreate() })
</script>

<style scoped>.sale-id-field{width:100%;display:flex;gap:8px}.sale-id-field .el-input{flex:1}.source-sale{margin:2px 0 12px}.return-lines{margin-top:4px}.return-lines header{padding:8px 0;display:flex;align-items:center;justify-content:space-between}.return-lines header small{color:var(--text-muted)}.return-lines :deep(.el-input-number){width:100%}</style>

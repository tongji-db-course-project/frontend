<template>
  <div class="data-page">
    ...
    <section v-loading="loading" class="card">
      <div class="toolbar">
        <label><Search /><input v-model="keyword" placeholder="搜索商品名称、条码" /></label>
        <select v-model="status"><option value="">全部状态</option><option>正常</option><option>停用</option></select>
        <button class="query" @click="search">查询</button><button @click="reset">重置</button><span>共 {{ total }} 件商品</span>
      </div>
      <div class="table-wrap"><table><thead><tr><th>商品信息</th><th>条码</th><th>分类 / 供应商</th><th>采购价</th><th>销售价</th><th>库存预警</th><th>状态</th><th>操作</th></tr></thead>
        <tbody><tr v-for="item in products" :key="item.productId">
          <td><div class="item-info"><i>{{ item.productName.slice(0,1) }}</i><div><b>{{ item.productName }}</b><small>{{ item.specification }} · {{ item.unit }}</small></div></div></td>
          <td><code>{{ item.barcode }}</code></td><td><b>{{ categoryName(item.categoryId) }}</b><small class="block">{{ supplierName(item.supplierId) }}</small></td>
          <td>¥ {{ item.purchasePrice.toFixed(2) }}</td><td><b>¥ {{ item.salePrice.toFixed(2) }}</b></td><td>{{ item.stockWarning }} {{ item.unit }}</td>
          <td><span class="status" :class="{off:item.status==='停用'}"><i />{{ item.status }}</span></td>
          <td class="actions"><button @click="openDetail(item)">详情</button><button @click="openForm(item)">编辑</button><button class="danger" @click="toggle(item)">{{ item.status==='正常'?'停用':'启用' }}</button></td>
        </tr></tbody></table></div>
...
const enabledCount=computed(()=>products.value.filter(x=>x.status==='正常').length)
...
async function search(){page.value=1;if(/^\d{8,}$/.test(keyword.value.trim())){loading.value=true;try{const item=await productApi.getByBarcode(keyword.value.trim());products.value=item?[item]:[];total.value=products.value.length}catch{products.value=[];total.value=0}finally{loading.value=false};return}loadProducts()}
...
async function openForm(item:Product|null=null){editingId.value=item?.productId||null;Object.assign(form,item?await productApi.getDetail(item.productId):empty());formVisible.value=true}
...
async function toggle(item:Product){if(item.status==='正常'){await ElMessageBox.confirm('停用后该商品将不可继续销售，确认停用吗？','停用商品',{type:'warning'});await productApi.remove(item.productId);ElMessage.success('商品已停用')}else{await productApi.update(item.productId,{...item,status:'正常'});ElMessage.success('商品已启用')}await loadProducts()}
async function openDetail(item:Product){selected.value=null;detailVisible.value=true;detailLoading.value=true;try{selected.value=await productApi.getDetail(item.productId)}finally{detailLoading.value=false}}
